...
void main() {
    ...
    float v1 = smoothstep(.9, 0.25, abs(vignetteCoords.x - 0.5));
    float v2 = smoothstep(.9, 0.25, abs(vignetteCoords.y - 0.4));
    ...
    float scale = 1.5; // Example scale factor, adjust based on desired effect strength
